...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def enemies(self):
            ...
            key: Place
            val: distance between ThrowerAnt and Bees
            """
            res = {}
            loc = 0
        ...
        if enemy == []:
            ...
            for e in enemy.keys():
                if self.lower_bound <= enemy[e] + 1 <= self.upper_bound:
                    return random_bee(e.bees)
            return None 
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            destroy = amount + self.damage
        
        index = len(self.place.bees) - 1
        while index >= 0:
            self.place.bees[index].reduce_health(destroy)
            index -= 1
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        index = len(self.place.bees) - 1
        while index >= 0:
            self.place.bees[index].reduce_health(self.damage)
            index -= 1
        ...
        while curr_place != None:
            if curr_place.ant != None:
                ...
                if curr_place.ant.is_container:
                    if curr_place.ant.ant_contained:
...
################
## EC1 Helper Function
def apply_status(status, bee, last_turns):
    original_action = bee.action
    new_action = status(bee.action, bee)

    def change_status(gamestate):
        nonlocal last_turns
        if last_turns > 0:
            last_turns -= 1
...
def make_scare(action, bee):
    def scared_without_slowed(gamestate):
        if bee.place.ant != None:
            action(gamestate)
        else:
            if not bee.place.entrance.is_hive:
                bee.move_to(bee.place.entrance)
    def scared_with_slowed(gamestate):
        if gamestate.time % 2 == 0:
            scared_without_slowed(gamestate)
    if bee.is_slowed:
        return scared_with_slowed
    return scared_without_slowed
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        if not self.was_scared:
            self.was_scaed = True
            apply_status(make_scare, self, 2)
        # END Problem EC 2
