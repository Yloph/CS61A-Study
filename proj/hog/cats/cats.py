...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        return res / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    res_words = [w for w in word_list if diff_function(typed_word, w, limit) <= limit]
    if res_words == []:
        return typed_word
    else:
        if typed_word in res_words:
            return typed_word

        def compare(word, compare_word = typed_word, l = limit):
            return diff_function(compare_word, word, l)
        res_word = min(res_words, key = compare)
        return res_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if len(typed) == 0 or len(source) == 0 or limit < 0:
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit < 0 or abs(len(typed) - len(source)) > limit: # 补长或删短的次数超过了limit, 无需递归
        return limit + 1
    if typed == source:
        return 0
    if typed == "" or source == "":
...
def final_diff(typed, source, limit):
    ...
    if limit < 0 or abs(len(typed) - len(source)) > limit:
        return limit + 1
    if typed == source:
        return 0
    if typed == "" or source == "":
        return max(len(typed), len(source))
    # 1.Leave out a letter that appears twice in a row
    if len(source) >= 2 and len(typed) >= 2 and source[0] == source[1] == typed[1] != typed[0]:
        return 1 + final_diff(typed, source[1:], limit - 1)
    # 2.Treat two adjacent letters that have swapped positions as one change
    if len(source) >= 2 and len(typed) >= 2 and (typed[0], typed[1]) == (source[1], source[0]):
        return 1 + final_diff(typed[2:], source[2:], limit - 1)
    # 3.Letters near to each other on the keyboard
    near_letters = {"q": 'wsa', "w": 'qasde', "e": 'wsdfr', "r": 'edfgt', "t": 'rrfghy', "y": 'tghju',
                    "u": 'yhjki', "i": 'ujklo', "o": 'iklp', "p": 'ol', "a": 'qwsz', "s": 'aqwedxz',
                    "d": 'werfcxs', "f": 'ertgvcd', "g": 'rtyhbvf', "h": 'tyujnbg', "j": 'yuikmnh',
                    "k": 'uiolmj', "l": 'iopk', "z": 'asx', "x": 'sdcaz', "c": 'sdfvx', "v": 'dfgbc',
                    "b": 'fghnv', "n": 'ghjmb', "m": 'hjkn'}
    if typed[0] != source[0] and typed[0] in near_letters.keys() and source[0] in near_letters[typed[0]]:
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        times.append([tpp[i][j + 1] - tpp[i][j] for j in range(len(tpp[i]) - 1)])
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if times[j][i] < times[fastest][i]:
                fastest = j
        fastest_player.append(fastest)
    
    res = []
    for i in player_indices:
        res.append([words[j] for j in word_indices if fastest_player[j] == i])
